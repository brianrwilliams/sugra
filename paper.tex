...
\d \colon \cE \to \cE [1] 
\]
such that (1) $\d^2 = 0$ and (2) $(\cE , \d)$ is an elliptic complex;
...
that is
\begin{enumerate}
\item fiberwise nondegenerate,
\item graded skew symmetric, and
\item satisfies $\int_M \omega\<e_0, \d e_1\> = (-1)^{|e_0|} \int_M \omega(\d e_0, e_1)$ where $e_i$ are compactly supported sections of $E$ .
\end{enumerate}
\end{itemize}
\end{dfn}
...
\begin{dfn}
A {\bf classical field theory} is a free classical field theory $(E, \d, \omega)$ equipped with an even functional
\[
I \in \oloc^+(E)
\]
...
Given a classical field theory $(E, \d, \omega, I)$ we denote by
\[S_{BV} = \frac{1}{2} \int_M \omega(e, \d e) + I\in \oloc(E)\]
the BV action of the theory.

\begin{remark}
We will also consider $\ZZ/2$-graded classical field theories which are defined as before, but where $E$ has only a single $\ZZ/2$-grading and, correspondigly, $\d$ is simply an odd operator.
\end{remark}
...
The classical master equation implies that the operator $\d + \{I,-\}$ squares to zero on $\cO(E)$, and hence every classical field theory defines a cochain complex
\[
\left(\cO(E), \d + \{I,-\}\right) .
\]
...
\begin{dfn}
A {\bf morphism} of classical field theories (defined on the same manifold $M$) $F\colon (E, \d, \omega, I) \to (E', \d', \omega', I')$ is a linear map of vector bundles
\[
F\colon E \to E'
\]
that intertwines the differentials $\d, \d'$, the pairings $\omega, \omega'$, and the interactions $I,I'$. 
\end{dfn}

Note that a map of classical field theories induces a map of BV complexes
\[
F^*\colon \left(\cO (\cE)[-1], \d + \{I,-\} \right) \to \left(\cO (\cE')[-1], \d' + \{I',-\} \right) .
\]
...
A morphism of classical field theories $F\colon (E, \d, \omega, I) \to (E', \d', \omega', I')$ is an {\bf equivalence} if it induces a quasi-isomorphism 
\[
F^* \colon  \left(\cO(\cE')[-1], \d' + \{I',-\} \right) \xto{\simeq} \left(\cO(\cE)[-1], \d + \{I,-\} \right) .
\]
\end{dfn}

\begin{dfn}
Let $(E, \d,\omega, I)$ be a classical field theory 
A {\bf strict action} of a super Lie algebra $\fg$ on $(E, \d,\omega, I)$ is a map of super dg Lie algebras
\[
\rho\colon \fg \to \left(\oloc(E)[-1] , \{-,-\}_\omega, \d + \{I,-\}_\omega\right) .
\]
More generally, an $L_\infty$ {\bf action} is an $\infty$-morphism of super dg Lie algebras
\[
\rho\colon \fg \rightsquigarrow \left(\oloc(E)[-1] , \{-,-\}_\omega, \d + \{I,-\}_\omega\right) .
\]
\end{dfn}
