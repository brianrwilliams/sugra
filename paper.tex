...
\begin{definition}
The \emph{BV antibracket} $\{-,-\}$ is the \chris{give at least an informal definition and a reference to a formal definition in the book \cite{Book2}.  Also cite \cite{WittenAntibracket}.}
\end{definition}
...
\subsection{Symmetries in the Classical BV Formalism}
...
\begin{definition}
\chris{The definition of the Chevalley-Eilenberg complex with coefficients, in particular the cup product / bracket, with its correct sign.}
\end{definition}
...
\chris{todo: replace the argument here with an argument that only works in terms of the $I^{(i)}$.}
...
We can rephrase Lemma \ref{onshell_action_lemma} in terms of these local functionals.  Let us write $\d_{\mr{CE}}^\Gamma$ for the Chevalley-Eilenberg differential associated to the \emph{supertranslation algebra} $\CC^m \oplus \Pi \Sigma$, with bracket given by the pairing $\Gamma$.  We include the superscript $\Gamma$ here to distinguish this differential from the Chevalley-Eilenberg differential in the full super Poincar\'e algebra, which we will use below.
...
\begin{proof}
\chris{fix to take care with signs, also we're removing that earlier lemma so this all needs changing.}
...
\end{align*}
Here we applied Lemma \ref{onshell_action_lemma} on the third line.
\end{proof}
...
\end{align*}
where $(123)$ indicates symmetrization in $Q_1, Q_2, Q_3$.  We can simplify this expression by manipulating the first summand.  Indeed,
\begin{align*}
\left\langle A^*\;,\; \Gamma(Q_3, \rho(\Gamma(Q_1,Q_2)) \lambda^*)\right\rangle &= 
\end{align*}
\chris{...}
