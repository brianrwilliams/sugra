...
Let $I_{\mr{rot}}$ be the interaction associated to the action of $\so(n)$ on super Yang-Mills theory from Theorem \ref{YM_Poincare_invariant_prop}.  The functionals $I^{(1)}$ and $I^{(2)}$ are rotation invariant, meaning that
\[\d_{\mr{Lie}} I^{(i)} = - \{I_{\mr{rot}}, I^{(i)}\},\]
for $i=1,2$.
\end{lemma}
