...
Our first example will generalize the fundamental example of \emph{BF} theory to a not entirely topological context.  Ordinarily, BF theory describes the classical BRST theory with fields \chris{... todo, include references.}
...
the integration map $\int_\CC \d z\wedge (-)\colon \Omega^{0, \bullet}(\CC)\rightarrow \CC$ reduces to the integration map $\int_\RR (-)\colon \Omega^{\bullet}(\RR;\CC)\rightarrow \CC$. \brian{This is not defined} \chris{is compactly supported enough?  We want to compare pairings.}
\end{proof}
...
The idea of twisting, originally developed by Witten \chris{refs}, is \chris{todo: brief contextual remarks, not much detail, as we'll say more in the intro.}
...
This datum generates a supersymmetry algebra $\mf A'$ in dimension $\mr{dim}(W_\RR)$ \chris{we never actually defined supersymmetry algebras or R-symmetry: need to go back and add this to Section \ref{sec:susyalgebras}.}.
...
% This is of course compatible with our conventions for holomorphic Chern-Simons: holomorphic Chern-Simons (with values in an ordinary Lie algebra) on an odd dimensional Calabi-Yau only defines a $\ZZ/2\ZZ$-graded theory (unless $d=3$, where this can be lifted to a $\ZZ$-grading). 
...
\begin{itemize}
 ...
 \item $U$-valued bosons: a pair of $W$-valued scalar fields $(\phi \otimes v,\phi' \otimes v') \in \Omega^0(\RR^3; (W \oplus W) \otimes U)$.
...
\vspace{-10pt}
\paragraph{Twisting data:}
In the $\mc N=(2,2)$ supersymmetry algebra there is are two classes of non-trivial orbits of square-zero supercharges.  We have both square-zero supercharges of rank $(1,0)$ or $(0,1)$, which are holomorphic, and square-zero supercharges of rank $(1,1)$, which are topological. \chris{todo, say something about $\ZZ$-gradings and twisting homomorphisms.}

\begin{thm} \label{2d_minimal_twist_thm}
The minimal twist of 2d $\cN=(2,2)$ super Yang-Mills with Lie algebra $\fg$ coupled to the $\cN=(2,2)$ chiral multiplet valued in a representation $R$ is perturbatively equivalent to topological $BF$ theory for the Lie algebra $\fg$ coupled to the mixed $\beta\gamma$ system with values in the representation $R$, with moduli space $T^*[-1]\mr{Map}(\RR^2_{\mr{dR}}, R/\gg)$. 
\end{thm}

\begin{proof}
This follows, as in 3d, by dimensionally reducing the holomorphic twist of 3d $\mc N=2$ super Yang-Mills.  That is, we start with the result from Theorem \ref{3d_minimal_twist_thm}, then apply Proposition \ref{CS_diml_red_prop} to turn the complex directions into a real de Rham direction.
\end{proof}

\subsection{\texorpdfstring{$\cN=(4,4)$}{N=(4,4)} Super Yang-Mills with matter} \label{sec:2d(4,4)}
We can likewise consider $\mc N=(4,4)$ super Yang-Mills theory with gauge group $G$, with a hypermultiplet valued in a symplectic representation $U$, by dimensionally reducing 3d $\mc N=4$ super Yang-Mills.

\subsection{\texorpdfstring{$\cN=(8,8)$}{N=(8,8)} Super Yang-Mills with matter} \label{sec:2d(8,8)}
