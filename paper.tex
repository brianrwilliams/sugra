...
\end{align*}
\chris{I should take care that the signs I introduce when I move $X$ around are all correct here, and include more explanation.  This argument assumes that $X \cdot \Gamma(Q_1, Q_2) = \Gamma(X \cdot Q_1, Q_2) + \Gamma(Q_1, X \cdot Q_2)$, that $(X \cdot Q_1, Q_2) + (Q_1, X \cdot Q_2) = 0$, and that $X \cdot (\rho(v)Q) = \rho(X \cdot v)Q + \rho(v)(X \cdot Q)$, which I think should follow from $\so(n)$ equivariance of $\Gamma$ and $\rho$, but I should check and verify the signs.}
\end{proof}
