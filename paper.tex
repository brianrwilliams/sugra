...
\[
Q_{BV} \colon \cE \to \cE [1] 
\]
such that $(1)$: $Q_{BV}^2 = 0$ and $(2)$: the pair $(\cE , Q_{BV})$ is an elliptic complex;
...
\item[$(3)$] satisfies $\int_M \omega(e_0, Q_{BV} e_1) = (-1)^{|e_0|} \int_M \omega(Q_{BV} e_0, e_1)$ where $e_i$ are compactly supported sections of $E$ .
\end{enumerate}
\end{itemize}
\end{definition}
...
This bracket together wtih $Q_{BV}$ endows $\oloc(\cE)[-1]$ with the structure of a dg Lie algebra. 
...
\[Q_{BV} I + \frac{1}{2} \{I,I\} = 0 .\]
\end{definition}

Given a classical field theory $(E, Q_{BV}, \omega, I)$ we denote by
\[S = \frac{1}{2} \int_M \omega(e, Q_{BV} e) + I\in \oloc(E)\]
the BV action of the theory.

\begin{remark}
The local functional $S$ satisfies the {\em classical master equation} \[\{S, S\} = 0.\] 
In fact, given a degree $(-1)$ nondegenerate pairing $\omega$ on $E$, prescribing the data of a classical field theory (namely $Q, I$ satisfying the Maurer-Cartan equation) is equivalent to prescribing a local functional $S \in \oloc(\cE)$, which is at least quadratic in the fields, satisfying the classical master equation.
The operator $Q_{BV}$ is the BV bracket with the quadratic part of the BV action $S$, and the cubic and higher terms in $S$ coincide with $I$.
...
\pavel{Chris, I think your diagrams of BV complexes would fit well here, can you insert them?}
...
In this section we define what it means for a (super) Lie algebra to act on a classical field theory. Let $(E, Q_{BV},\omega, I)$ be a classical field theory and $\fg$ a super Lie algebra. Consider the graded vector space
...
The operator $Q_{BV}$ on $\oloc(\cE)$ extends $C^\bu(\fg)$-linearly to an operator on $C^\bullet(\fg, \oloc(\cE))$ by the rule
\[
(Q_{BV} f)(x_1,\ldots, x_n) = Q_{BV} f(x_1,\ldots, x_n) 
\]
where $f : \fg^{\otimes n} \to \oloc(\cE)$. 
The differentials $\d_{CE}$ and $Q_{BV}$ are compatible in the sense that $(\d_{CE} + Q_{BV})^2 = 0$ making $C^\bu(\fg, \oloc(\cE))$ into a cochain complex with total differential $\d_{CE} + Q_{BV}$. 
Via the BV bracket, the shift of this cochain complex $C^\bu(\fg, \oloc(\cE))[-1]$ is a dg Lie algebra.

\begin{definition}
\label{infinitesimal_action_def}
Let $(E, Q_{BV},\omega, I)$ be a classical field theory and $\fg$ a super Lie algebra. 
\begin{itemize}
\item[(1)]
An $L_\infty$ \defterm{action} of a super Lie algebra $\fg$ on $(E, Q_{BV}, \omega, I)$ is an element of cohomological degree zero 
...
\[(\d_{CE} + Q_{BV}) I_\fg + \frac{1}{2} \{I_\fg, I_\fg\} = 0.\]
\end{itemize}
\item[(2)]
...
By the Maurer-Cartan equation, the operator $\d_{CE} + Q_{BV} + \{I_\fg, -\}$ 
...
Let $(E, Q_{BV}, \omega, I)$ be a classical field theory and $G$ is a supergroup acting on spacetime $M$. An (elliptic) \defterm{action} of $G$ on $(E, Q_{BV}, \omega, I)$ is given by the following data:
\begin{itemize}
...
\[
I = I_{F} + p^* I_{BRST} \in \oloc^+(\cE) .
\]

\begin{lemma}
Suppose $(F, S_{BRST})$ is a classical BRST theory such that $(\cE, Q)$ defined above is an elliptic complex. Then $(E, Q_{BV}, \omega, I)$ is a classical BV theory.
\end{lemma}

We refer to the classical BV theory $(E, Q_{BV}, \omega, I)$ from the above statement as the \defterm{$(-1)$-shifted cotangent bundle} of $\cF$ and by abuse of notation we often denote it simply by $\cE = T^*[-1] \cF$. In the case $S_{BRST} = 0$ we refer to the theory $T^*[-1] \cF$ as a theory of \defterm{cotangent type}.
...
\brian{Is this quite right? I think $\alpha$ is something different.
Physics usually calls the twisting homomorphism the one from the Lorentz group to $G_R$. 
Also, I think $Q$ is required to be a scalar under the twisting homomorphism.}
...
Suppose $(E, Q_{BV}, \omega, I)$ is a supersymmetric classical field theory and $(Q, \alpha)$ is a twisting datum. The \defterm{$Q$-twisted classical field theory} is the classical field theory 
\[
\left(E^Q, Q_{BV} + \{I^2_{\fA} (Q), -\}, \omega, I + I^{\geq 3}_\fA (Q)\right)
\]
where
