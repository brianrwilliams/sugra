...
We call $\cE$ the \defterm{space of BV fields}.  
The pairing $\omega$ equips the algebra of local functionals on $E$ with a shifted bracket, in the following way. In \cite[Chapter 5, Section 3]{CostelloBook}, it is shown that to every functional $f \in \oloc(\cE)$ there exists a symplectic vector field $X_f$ on $\cE$ whose Hamiltonian function is $f$. 
Note that if $f$ is degree $n$, then $X_f$ is of degree $n+1$ due to the degree of $\omega$. 

\begin{definition}
The \defterm{BV bracket} $\{-,-\}$ is the bilinear map of cohomological degree $+1$
\[
...
\begin{remark}
Note that the BV bracket in fact defines a bilinear map $\{-, -\}\colon \oloc(\cE)\times\cO(\cE)\rightarrow \cO(\cE)$. However, it does \emph{not} define a bracket on $\cO(\cE)$ itself.
\end{remark}
...
A local functional $I$ induces an endomorphism $\{I,-\}$ on the space of all functionals $\cO(E)$. The classical master equation implies that the operator $Q + \{I,-\}$ squares to zero on $\cO(\cE)$, and hence every classical field theory defines a cochain complex
...
where $S_0$ and $S_1$ are independent of $\phi$. Then the theory $(E, Q, \omega, I)$ is perturbatively equivalent to the theory $(E_0, Q_0, \omega_0, I_0)$ with the BV action $S_0 - S_1^2/2$, where we set $\phi^* = 0$ and $\phi = S_1$.
\label{prop:integrateoutfield}
\end{prop}
\begin{proof}
\pavel{The map $E_0\rightarrow E$ is obviously compatible with all the structures.}
\end{proof}

We may also remove a trivial BRST doublet.

\begin{prop}
Suppose $(E, Q, \omega, I)$ is a classical field theory, $F\rightarrow M$ is a graded vector bundle, such that $E\cong E_0\oplus F\oplus F^![-1]\oplus F^!\oplus F[-1]$. Denote by $\phi, \phi^*$ sections of $F,F^![-1]$ and by $\psi, \psi^*$ sections of $F^!, F[-1]$. Suppose
\[Q\phi + \{I, \phi\} = \psi - F_\psi,\qquad Q\psi^* + \{I, \psi^*\} = \phi^* - F_{\phi^*},\]
where $F_\psi$ is independent of $\psi$ and $F_{\phi^*}$ is independent of $\phi^*$. Then the theory $(E, Q, \omega, I)$ is perturbatively equivalent to the theory $(E_0, Q_0, \omega_0, I_0)$ with the BV action obtained by setting $\phi = 0, \phi^* = F_{\phi^*}, \psi = F_\psi, \psi^* = 0$ in the original BV action.
...
Fix a Lie algebra $\fg$ equipped with a nondegenerate symmetric bilinear pairing $\langle -, -\rangle$ and a Clifford module $\Sigma\oplus \Sigma^*$ as in Section \ref{sect:spinors}. The fields of the Yang--Mills theory are as follows:
\begin{itemize}
...
\[S(A, \lambda) = \int_{V_\RR} \left\langle -\frac{1}{4} F_A \wedge \ast F_A + \frac{1}{2}(\lambda, \sd \dd_A \lambda)\right\rangle.\]
...
\begin{equation}
S_{BV} = \int_{V_\RR} \left\langle -\frac{1}{4} F_A \wedge \ast F_A + \frac{1}{2}(\lambda, \sd\dd_A \lambda)\right\rangle - (\d_A c, A^*) + ([\lambda, c], \lambda^*) + \frac{1}{2}([c, c], c^*).
\label{eq:YMBVaction}
\end{equation}
...
\begin{definition}
The \defterm{Poincar\'e group} is $\mr{ISO}(V_\RR) = \Spin(V_\RR) \ltimes \RR^n$. The \defterm{Poincar\'e algebra} is its complexified Lie algebra $\mf{iso}(V)$.
\end{definition}
...
\item (8 supercharges). This theory exists in dimensions 2 through 6 and it depends on a Lie algebra $\fg$ together with a symplectic $\fg$-representation $U$. \pavel{Moment maps?}
...
\begin{align*}
\{S_{BV}^2, I^{(1)}(Q)\} &= -\frac{1}{2}(\lambda, \rho(\Gamma(Q, \lambda))\lambda) + \frac{1}{2}(\rho(F_A) Q, \sd{\dd}_A\lambda) \\
&= -\frac{1}{2}(\Gamma(Q, \lambda), \Gamma(\lambda, \lambda))  - \frac{1}{2}(\lambda, \sd{\dd}_A(\rho(F_A) Q)) \\
...
By \eqref{eq:Gammaspinorpairing} and Theorem \ref{thm:3psi} we have $(\Gamma(Q, \lambda), \Gamma(\lambda, \lambda)) = 0$, so $\{S_{BV}^1 + S_{BV}^2, I^{(1)}(Q)\} = 0$.
...
and
\begin{align*}
\{S_{BV}, I^{(2)}(Q_1, Q_2)\} = &-\frac{1}{2}(Q_2, \lambda^*)(Q_1, \sd{\dd}_A \lambda + [c, \lambda^*]) - \frac{1}{2}(Q_1, \lambda^*)(Q_2, \sd{\dd}_A \lambda + [c, \lambda^*]) \\
&+\frac{1}{2}(\Gamma(Q_1, Q_2), \Gamma(\lambda^*, \sd{\dd}_A \lambda + [c, \lambda^*])) + \iota_{\Gamma(Q_1, Q_2)}(d_A c) c^* + d_A\iota_{\Gamma(Q_1, Q_2)} A\wedge A^* \\
...
&+ \frac{1}{2}\rho(\Gamma(Q_1, Q_2))\sd{\dd}_A\lambda - \frac{1}{2}(Q_2, \sd{\dd}_A\lambda) Q_1 -\frac{1}{2}(Q_1, \sd{\dd}_A\lambda) Q_2 + [\lambda, (\Gamma(Q_1, Q_2), A)]
\end{align*}
Using Proposition \ref{prop:cliffordactionproperty2} the first, second, fifth and sixth terms combine to
\[-\frac{1}{2} \sd{\dd}_A\rho(\Gamma(Q_1, \lambda))Q_2 - \frac{1}{2} \sd{\dd}_A\rho(\Gamma(Q_2, \lambda)) Q_1\]
which is equal to $\frac{1}{2} \sd{\dd}_A \rho(\Gamma(Q_1, Q_2))\lambda$ by Theorem \ref{thm:3psi}. Using the Clifford relation this term cancels the rest of the terms.
\end{proof}
...
$\{I^{(1)}, I^{(2)}\}(Q_1, Q_2, Q_3)$ is obtained by cyclically symmetrizing the above expression. By Theorem \ref{thm:3psi} the cyclic symmetrization of the term with $c^*$ is zero. The Clifford relation implies that
\begin{align*}
...
\[\Gamma_P\colon \Sym^2(\Sigma^*\otimes_A P)\rightarrow V\]
defined by the property
\[(v, \Gamma_P(\psi_1, \psi_2)) = (\psi_1, \rho(v) \psi_2),\qquad v\in V,\ \psi_i\in \Sigma^*\otimes_A P.\]

The action for the matter multiplet is
\[
S = \int_{V_\RR} -\frac{1}{2}  (\d \phi \wedge * \d \phi) + \frac{1}{2} (\psi , \sd \dd \psi)\dvol,
\]
...
\begin{align*}
I^{(1)} (Q) & = \int (\phi^*, (Q, \psi)) - (\psi^*, \rho(\d \phi) Q) \\
I^{(2)} (Q_1 , Q_2) & = \int \frac{1}{4}(\Gamma(Q_1, Q_2) , \Gamma(\psi^*, \psi^*)_P) \end{align*}
where $Q, Q_1,Q_2 \in \Sigma$.

The following result states that these functionals encode an off-shell action of the supersymmetry on the matter multiplet.

\begin{thm}
The functional $\fS = S + I^{(1)} + I^{(2)}$ satisfies the classical master equation
\begin{equation}\label{CMEmatter}
\d_{\rm Lie} \fS + \frac{1}{2} \{\fS, \fS\} = 0 .
\end{equation}
\end{thm}

We decompose the classical master equation \eqref{CMEmatter} into the following equations:
\begin{equation}\label{CMEmatter2}
\begin{array}{rrrrrr}
\{S , I^{(1)}\} & = & 0 \\ 
...
The first equation in \eqref{CMEmatter2} states that the classical action for the matter multiplet is supersymmetric. 

\begin{lemma} \label{lem: matter1}
One has $\{S, I^{(1)}\} (Q) = 0$ for all $Q \in \Sigma$. 
\end{lemma}
\begin{proof}
Observe
\[
\{(\d \phi , \d \phi), (\phi^* , (Q, \psi)) \} = 2 (\d (Q, \psi) , \d \phi ) .
\]
and
\[
\{(\psi, \sd \dd \psi) , (\psi^*, \rho(\d \phi) Q) \} = 2(\psi, \sd \dd \rho(\d \phi) Q).
\]

By Proposition \ref{prop:cliffordactionproperty3} $\sd \dd\rho(\d\phi) Q = (\d^* \d\phi) Q$. Therefore,
\[2(\psi, \sd \dd \rho(\d \phi) Q) = 2(\d^*\d\phi)(\psi, Q) = -2(\d\phi, \d(\psi, Q))\]
which cancels the first term.
\end{proof}

Next, we move on to the second equation in \eqref{CMEmatter2}. 

\begin{lemma} 
One has
\begin{equation}\label{CMEmatter3}
\{S, I^{(2)}\} + \d_{CE} I^{(1)} + \frac{1}{2} \{I^{(1)}, I^{(1)}\} = 0 .
\end{equation}
\end{lemma}
\begin{proof}
Evaluating expression \eqref{CMEmatter3} on $v_1,v_2 \in \mf{iso}(V)$ reduces to the claim that (??) defines a strict Lie action. 
...
The individual terms are:
\begin{align*}
\frac{1}{2}\{I^{(1)}, I^{(1)}\}(Q_1, Q_2) = &\phi^* (Q_1, \rho(\d \phi) Q_2) + \phi^* (Q_2, \rho(\d \phi)Q_1) \\ &  + (\psi^*, \rho(\d(Q_1,\psi))Q_2) + (\psi^*, \rho(\d(Q_2, \psi)) Q_1) 
\end{align*}
...
and
\begin{align*}
\{S, I^{(2)}(Q_1,Q_2)\} = & \frac{1}{2} \Gamma(Q_1,Q_2) \Gamma(\psi^*, \sd \dd \psi)_P
\end{align*}
Note that we can rewrite this as $\frac{1}{2} (\rho(\Gamma(Q_1, Q_2)) \sd \dd \psi, \psi^*)$. 

We first collect all terms in equation \eqref{CMEmatter3} proportional to $\phi^*$:
\[
\frac{1}{2} (Q_1, \rho(\d\phi) Q_2) + \frac{1}{2} (Q_2, \rho(\d \phi) Q_1) - L_{\Gamma(Q_1,Q_2)} \phi .
\]

By \eqref{eq:Gammaspinorpairing} we observe that the first two terms cancel with the third term.

Next, we collect all terms in equation \eqref{CMEmatter3} proportional to $\psi^*$:
\begin{equation}\label{psistar}
\frac{1}{2} \sum_i (\rho(e^i)Q_2)(Q_1,\partial_i \psi)^A + \frac{1}{2} \sum_i (\rho(e^i) Q_1)(Q_2, \partial_i\psi)^A - \Gamma(Q_1,Q_2) . \psi + \frac{1}{2} \rho(\Gamma(Q_1,Q_2)) \sd \dd \psi.
\end{equation}
The first and second terms can be written as:
\[
\frac{1}{2} \sum_i (\rho(e^i)Q_2)(Q_1, \partial_i \psi)^A + \frac{1}{2} \sum_i(\rho(e^i)Q_1)(Q_2, \partial_i \psi)^A = \frac{1}{2} \sd \dd \left(Q_2(Q_1, \psi)^A + Q_1(Q_2, \psi)^A\right).
\]
Applying Theorem \ref{thm:matter3psi} to $Q_3 = \psi$, this becomes $\frac{1}{2} \sd \dd \rho(\Gamma(Q_1,Q_2) \psi)$. 
Finally, by the Clifford identity the sum of this term with the fourth term in \eqref{psistar} is precisely $\Gamma(Q_1,Q_2) . \psi$ which cancels the third term.
\end{proof}
...
We have
\begin{align*}
\{I^{(1)}(Q_1), I^{(2)}(Q_2, Q_3)\} & = \frac{1}{2} (\Gamma(Q_2, Q_3), \Gamma(\psi^*, \phi^* Q_1)_P) \\ & = (\psi^*, \phi^* \rho(\Gamma(Q_2,Q_3)) Q_1)  .
\end{align*}
The expression
$\{I^{(1)}, I^{(2)}\}(Q_1,Q_2,Q_3)$ is obtained by cyclically symmetrizing the above expression. By Theorem \ref{thm:3psi} the cyclic symmetrization is identically zero.
\end{proof}
...
and
\[
\{(\psi, \sd \dd \psi) , (\psi^*, \rho(\d \phi) Q) \} = 2 (\rho(\d \phi) Q, \sd \dd \psi) .
\]
...
% \textcolor{red}{- \frac{1}{2} (Q_1, \psi^*) (Q_2, \sd \dd \psi) - \frac{1}{2} (Q_1, \sd \dd \psi) (Q_2, \psi^*) }.
\end{align*}
...
%\textcolor{red}{ - \frac{1}{2} (Q_2, \sd \dd \psi)Q_1 - \frac{1}{2} (Q_1, \sd \dd \psi)Q_2}
\end{array}
\end{equation}
By Proposition \ref{prop:cliffordactionproperty3}, the first and second terms can be written as:
\[
...
The holomorphic Chern--Simons theory has fields $c, A_{0, 1}, B, \tilde{B}, \tilde{A}_{0, 1}, \tilde{c}$ with the BV action
\[
S_{CS} = \int \left(c\wedge \ol\dd_{A_{0, 1}} \tilde{A}_{0, 1} + \tilde{B}\wedge F_{0, 2} + \frac{1}{2} B\wedge \ol\dd_{A_{0, 1}} B + \frac{1}{2}[c, c]\tilde{c} + [B, c]\wedge \tilde{B}\right)\Omega
\]
Up to a rescaling of the antifields, it coincides with the BV action $S_0$.
\end{proof}
...
\subsection{Dimension 7} \label{7d_section}
...
\subsubsection{$\mc N = (1,0)$ Super Yang-Mills Theory with Matter} \label{6d_10_section}
...
\subsubsection{$\mc N = 2$ Super Yang-Mills Theory} \label{5d_2_section}
...
\[\sd{\dd}\colon \Gamma(M, \Sigma)\rightarrow \Gamma(M, \Sigma^*).\]
...
\[\sd{\dd}\rho(\Gamma(Q_1, \lambda)) Q_2 = \rho(d \Gamma(Q_1, \lambda)) Q_2 + (Q_1, \sd{\dd} \lambda) Q_2.\]
...
\[\sd{\dd}(\rho(X) Q) = \rho(\d X)Q + (-1)^{n(1+p)}\rho(\ast \d\ast X) Q.\]
\label{prop:cliffordactionproperty3}
\end{prop}

Note that both Proposition \ref{prop:cliffordactionproperty2} and Proposition \ref{prop:cliffordactionproperty3} extend to the case when $\lambda$ and $X$ respectively are twisted by a vector bundle and $\sd{\dd}$ is the corresponding twisted Dirac operator.
