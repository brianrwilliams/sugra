...
\begin{thm}
The twist of $(4+\dim(L))$-dimensional super Yang--Mills on $L\times N_\RR$ by $Q$ is perturbatively equivalent to the generalized BF theory with the space of fields $\map(L\times (N_\RR)_{\mr{dR}}, B\fg)$. Moreover, the equivalence is $\MU(L)\times \Spin(N_\RR)$-equivariant.
...
Fix a K\"ahler structure on $\RR^4$ and denote by $L\subset V$ the $i$-eigenspace of the complex structure determined by the holomorphic supercharge $Q_0$. %Moreover, fix a complex volume form on $L$. 
...
\begin{itemize}
 \item $\gg$-valued Bosons: a gauge field $A \in \Omega^1(\RR^4; \gg)$, and a pair of scalar fields $(a \otimes \alpha_-^{\otimes 2} ,\Tilde{a} \otimes \alpha_+^{\otimes 2}) \in \Omega^0(\RR^4; (S^2_-)^{\otimes 2} \gg \oplus  (S^2_-)^{\otimes 2} \otimes \gg)$.
 \item $U$-valued bosons: a $W = W_+ \otimes S_+^2$-valued scalar field $\phi \otimes v \in \Omega^0(\RR^4; W_+ \otimes S_+^2 \otimes U)$.
 \item $\gg$-valued fermions: a $W = W_+ \otimes S_+^2$-valued Dirac spinor $\lambda = (\lambda_+ \otimes \alpha_+, \lambda_-\otimes \alpha_-) \in \Omega^0(\RR^4; (S_+ \otimes W \oplus S_- \otimes W^*) \otimes \gg)$.
...
\brian{If we split $V^{6d}$ as $V^{4d} \oplus N \oplus N^*$ where $N, N^*$ are trivial one-dimensional $\Spin(4)$-representations, then $W^{4d} = W_+^{6d} \otimes N$ and $W^{4d*} = W_+^{6d} \otimes N^*$. 
Moreover, the isomorphism $\wedge^2 S_+^{6d} \cong V^{6d}$ gives rise to three maps
\begin{itemize}
\item $S^{4d}_+ \otimes S^{4d}_- \otimes N \otimes N^* \to V^{4d}$. This is $\Gamma^{4d} \otimes {\rm ev}_N$. 
\item $\wedge^2(S_+^{4d} \otimes N) \to N^*$. 
\item $\wedge^2(S_-^{4d} \otimes N^*) \to N$. 
\end{itemize}
The bosons in the gauge part live in $a \in N^*$ and $\Tilde{a} \in N$. 
For $Q_+ \in S^{4d}_+ \otimes W^{4d} = S^{4d}_+ \otimes W^{6d}_+ \otimes N$ and $\lambda_+ \in S^{4d}_+ \otimes W^{4d} = S^{4d}_+ \otimes W^{6d}_+ \otimes N$. 
I want to make sense of $(\<Q_+, \lambda_+\> , \Tilde{a}^*)$ as a scalar via the composition
\[
\wedge^2 (S_+^{4d} \otimes W^{6d}_+ \otimes N) \otimes N  \xto{\omega_{W_+}} \wedge^2(S^{4d}_+ \otimes N) \otimes N \to N^* \otimes N \to \CC .
\]
The first map is the symplectic pairing on $W_+$. 
The second map is the second bullet above. 
The last map is evaluation.
Do you see an easier way to canonically write this using only 4d objects?
}
