...
The holomorphic twist of 8d $\mc N=1$ super Yang-Mills theory on a Calabi-Yau 4-fold $X_4$ is equivalent to holomorphic BF theory on $X_4$.
\end{theorem}

\begin{proof}
We can demonstrate this by applying Lemma \ref{CS_to_BF_diml_red_lemma} to the minimal twist of 9d super Yang-Mills theory, as calculated in Theorem \ref{9d_twist_thm}.  Indeed, \chris{...}
\end{proof}
...
\chris{Here we're going to decompose the Yang-Mills fields under $\spin(7)$.  The vector of $\so(8)$ becomes the 8d Dirac spinor representation, and the two Weyl spinor representations both split as $7+1$, so the fields look like $\psi$, $A_+, A_-, \phi_+, \phi_-$.}
