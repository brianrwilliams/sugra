...
\chris{before the definition below we should probably define the antibracket $\{,\}$ (in terms of the shifted symplectic structure).}
...
\[S_{BV} = \frac{1}{2} \int_M \omega(e, Q e) + I\in \oloc(E)\]
...
\[\{S_{BV}, I^{(k+1)}\} + \d_{CE} I^{(k)} + \frac{1}{2}\sum_{a+b=k+1} \{I^{(a)}, I^{(b)}\} = 0\]
for every $k\geq 0$.
...
\end{align*}
since \chris{The equality of the first terms looks right when I write it in indices, not sure how to write it coordinate freely.  Also not sure of the sign of the second term.}.
