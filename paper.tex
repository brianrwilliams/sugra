...
\end{prop}
\begin{proof}
We first write down the functionals in $\Spin(4)$-notation.
The fields of $4d \cN=2$ decompose as:
\begin{itemize}
 \item $\gg$-valued Bosons: a gauge field $A \in \Omega^1(\RR^4; \gg)$, and a pair of scalar fields $(a ,\Tilde{a}) \in \Omega^0(\RR^4; \gg \oplus \gg)$.
...
For a general supercharge $Q_+ \in S_+ \otimes W$, the functionals $I_{\rm gauge}^{(1)}, I^{(1)}_{\rm matter}, I^{(2)}_{\rm gauge}, I^{(2)}_{\rm matter}$ decompose as:
\begin{align*}
I^{(1)}_{\mr{gauge}}(Q_+) &= \int \dvol \bigg(-( (\Gamma(Q_+,\lambda_-), A^*) + \langle Q_+,\lambda_+\rangle w_+ \wedge u_+, \phi_2^* ) + \\
