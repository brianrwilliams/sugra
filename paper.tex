...
\chris{Todo: would be more correct to remove the generic rank 2 twist in 3d, same as the 3d A twist (dual RW).  I should do this in the tables and the diagrams.  Also todo: change the labels of orbits to A and B.}
...
the integration map $\int_V \Omega_V\wedge (-)\colon \Omega^{0, \bullet}(V)\rightarrow \CC$ reduces to the integration map $\int_{V_\RR} (-)\colon \Omega^{\bullet}(V_\RR;\CC)\rightarrow \CC$. \brian{This is not defined}
\end{proof}
...
It is known \cite[Chapter 1.3]{SpringerVeldkamp} that the data of the antiinvolution $\sigma$ may equivalently be encoded in the data of a non-degenerate multiplicative norm $x\mapsto x\sigma(x)$, i.e. $A$ is a complex composition algebra.
...
In the intermediate case we refer to $Q$ as a \defterm{holomorphic-topological} (alternatively, partially topological) supercharge. The collection of all square-zero supercharges in dimensions 2 through 10 (where one restricts to supersymmetries with at most 16 supercharges) was studied in \cite{ElliottSafronov} and \cite{EagerSaberiWalcher}. In particular, orbits of square-zero supercharges under the $R$-symmetry group, $\Spin(V_\RR)$ and the obvious scaling action of $\CC^\times$ are shown in Figure \ref{fig:superchargeorbits}:
\begin{itemize}
...
\end{itemize}
We also introduce their antifields $A^*\in\Omega^1(V_\RR; \Pi\fg)$, $\lambda\in\Gamma(V_\RR; \Sigma^*\otimes \fg)$ and $c^*\in\Gamma(V_\RR; \fg)$. Let $\cE_{\gauge}$ denote the space of BV fields. 
...
\subsection{$\cN=(1, 0)$ super Yang--Mills}
...
\begin{itemize}
\item A square-zero supercharge $Q\neq 0\in\Sigma$ has 5 invariant directions and does not admit a compatible twisting datum $\alpha$. So, it gives rise to a $\ZZ/2$-graded holomorphic theory. A way to construct such a supercharge is to choose a complex structure on $\RR^{10}$ together with a nonvanishing complex half-density whose square is a holomorphic volume form $\Omega\in\Omega^{5, 0}(\CC^5)$. Such a supercharge is stabilized by $G=\SU(5)\subset \Spin(10, \CC)$.
\end{itemize}
...
\subsection{$\cN=1$ super Yang--Mills}
...
\subsection{$\cN=(0,4)$ Super Yang-Mills with matter}
...
\item $\fg$-valued bosons: a gauge field $A \in \Omega^1(\RR^2;\fg)$.
\item $\fg$-valued fermions: a spinor $\lambda \in C^\infty(\RR^2 ; S_- \otimes W_- \otimes \fg)$. 
\item $W_- \otimes U$-valued scalars: $\phi \in C^\infty(\RR^2 ; W_- \otimes U)$;
\item $U$-valued fermions: $\psi \in C^\infty(\RR^2 ; S_+ \otimes U)$ .
\end{itemize}
Here, $W_-$ is a complex four-dimensional vector space equipped with a symmetric pairing. 
Choose the basis $\{u_1,u_2,v_1,v_2\}$ for $W_-$ with pairing given by $\<u_i, v_j\> = \delta_{ij}$. 

% \cN=(0,8) Super Yang-Mills with matter

\subsection{$\cN=(0,\cN_-)$ Super Yang-Mills}

\begin{itemize}
\item $\fg$-valued bosons: a gauge field $A \in \Omega^1(\RR^2;\fg)$.
\item $\fg$-valued fermions: a spinor $\lambda \in C^\infty(\RR^2 ; S_- \otimes W_- \otimes \fg)$. 
\end{itemize}
Choose a splitting $W_- = {\rm span}_{\CC} \{u, v\} \oplus W_-'$ where $\dim(W_-') = \cN_- - 2$.
