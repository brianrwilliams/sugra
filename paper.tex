...
When we dimensionally reduce the minimally twisted 10- or 9-dimensional super Yang-Mills theories to 8 dimensions we obtain either the twist by a supercharge of type 1, or of type 3, depending on whether we reduce along an invariant direction or a non-invariant direction. \chris{I should say this more clearly, maybe including some general discussion at the beginning of the section, and referring to the figure.}
...
\chris{There might be a clever trick to avoid these calculations, involving the 10d calculation we already did to argue that certain fields definitely don't contribute (the ones that died in the 10d holomorphic twist, so we're left with only $A_{0,1}, \chi^0_1, \phi_1, \psi^+_{0,1}, \psi^+_{1,0}, \psi^-_{0,1}$ and $\chi^-_1$.) -- I need to think about it.}
...
The 4-dimensional $\mc N=1$ pure super Yang-Mills theory has BRST fields given by a ghost $c$, a gauge field $A$, and a Lie algebra valued Weyl spinor field $\lambda$. 
In addition, $4$-dimensional supersymmetry supports a chiral (anti-chiral matter) multiplet, which comprises a complex scalar $\phi$ and a left (right) Weyl spinor $\psi_+$ ($\psi_-$). 
...
We have written down the action for supersymmetric Yang-Mills numerous times. 
The kinetic action for $4$-dimensional $\mc N = 1$ matter is of the form
\[
S_{\rm matter} (\phi, \Bar{\phi}, \psi_+, \psi_-) = \int \d^4 x \; \<\partial_{x_i} \phi , \partial_{x_i} \Bar{\phi}\>_R + \int \d^4 x \; \<\psi_+ , \sd \dd \psi_-\>_R .
\]
Here $\<-,-\>_R$ denotes the obvious pairing between $R$ and $R^*$. 
The coupling to Super Yang-Mills couples to the matter via the action \brian{what to say}
\[
S_{\rm couple} = \int \d^4 x \;
\]
Let $S_{\rm BV}$ denote the full BV action of $\mc N = 1$ super Yang-Mills coupled to matter.
...
As in Section \ref{symmetry_section}, we encode the action by supersymmetry for the $4$-dimensional matter theory by the functionals
\begin{align*}
I^{(1)}_{\rm matter} (Q_+, Q_-) & = \int \<\phi^*, (Q_+, \psi_+)\>_R + \int \<\Bar{\phi}^*, (Q_-, \psi_-)\>_R + \int \<\psi^*_+, Q_+ \d \phi\>_R + \int \<\psi_-^*, Q_- \d \Bar{\phi}\>_R \\
I^{(2)}_{\rm matter} (Q_+ \otimes Q_-) & = \int \<\psi^*_- , \rho(\Gamma(Q_+, Q_-)) \psi_+\>_R 
\end{align*}
where $Q_\pm \in S_{\pm}$. 

Also define $I^{(1)}_{\rm gauge}$ and $I^{(2)}_{\rm gauge}$. \brian{this is so similar to the 10d case. How do you suggest we do this?}
...
We proceed to describe the twist of this general $4$-dimensional $\mc N=1$ gauge theory. 
There is a single non-zero equivalence class of square zero supercharges in $4$-dimensional $\mc N = 1$, which we fix from hereon. 
Such supercharges are minimal (i.e. have 2d image).  
They are stabilized by $\SU(2)$ \brian{really $U(2)$, i think}.  
The R-symmetry group is $U(1)$, and so the twisted theory by such a supercharge is always $\ZZ$-graded.
