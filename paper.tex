...
We consider $\cN=(0,2)$ super Yang-Mills coupled to the $\cN=(0,2)$ chiral multiplet with values in a representation $R$. 
...
\begin{thm}[See also \cite{SWchar}] \label{4d_minimal_twist_thm}
The minimal twist of 2d $\cN=(0,\cN_-)$ super Yang-Mills with Lie algebra $\fg$ is $U(1)$-equivariantly equivalent to holomorphic $BF$ theory on $\CC$ for the Lie algebra $\fg$ coupled to a free fermion system on $\CC$ with values in the representation $\fg \otimes W_-' \cong \fg \otimes \fg \otimes \CC^{\cN_- - 2}$ where $\fg$ acts on the left by the adjoint action. 
\end{thm}
\begin{proof}
First, we integrate out the fields $\lambda_{0}$ and $\lambda_{1,1}^*$ using Proposition \ref{prop:integrateoutfield}.  
We then observe that the action includes the terms $\int  (\lambda_{1,0} , A_{1,0}^*)$ and  $\int (\phi, \psi_0^*)$.  
