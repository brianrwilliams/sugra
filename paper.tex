...
\begin{prop}
Let $S_{\mr{BV}}$ be the BV action functional in the theory $\fL$, and write $I_{\mr{Poin}}$ for the interaction generating the Lie action of the Poincar\'e algebra \chris{we'll have to see whether we want Poincar\'e or just translation here, probably translation, because then there are no $\d_{\mr{Lie}}I^{(i)}$ terms to worry about.  On the other hand we would like to know that the theories are supersymmetric, not just super-translation invariant.  To include rotations and R-symmetries we need to additionally check that some terms like $\d_{\mr{Lie}}+ \{I_{\mr{rotation}} + I_{\text{R-symmetry}}, I^{(i)}\}$ vanish.}. The functional
