...
\begin{eg}
There is a special case of this  of the ``holomorphic symplectic boson". 
\brian{include this?}
\end{eg}
...
\section{Dimension 6}
In dimension 6 the odd part of the supersymmetry algebra is $\Sigma = S_+ \otimes W_+$.
There is a minimal supersymmetric field theory with $\mc N=(1,0)$ supersymmetry. 
Here, $W_+$ is a two-dimensional complex vector space equipped with a symplectic form. 
...
Let $\fg$ be a complex Lie algebra equipped with a symmetric bilinear invariant nondegenerate pairing. 
The general theory with $\mc N = (1,0)$ supersymmetry consists of pure super Yang-Mills with Lie algebra $\fg$ coupled to $(1,0)$ matter valued in a symplectic vector space $U$, as in Section \ref{sect:mattermultipletSUSY}. 
In physics terminology, the matter here is referred to as the $6d$ $\mc N = (1,0)$ hypermultiplet.

The $R$-symmetry group is $G_R = \SU(2)$. \brian{Or do we want it's noncompact form ${\rm Sp}(W_+) = {\rm Sp}(2, \CC)$. }

\vspace{-10pt}
\paragraph{Fields:}
The fields of the theory are given by:
\begin{itemize}
...
Under the $R$-symmetry $\lambda, \phi$ transform as the fundamental representations. \brian{Does this sound right?}

\subsubsection{Holomorphic twist}

Choose a polarized basis $W_+ = {\rm span}_{\CC}\{w_1, w_2\}$.
The only square-zero supercharges in the $\mc N=(1,0)$ supersymmetry algebra have rank one.  
Without loss of generality we will twist with respect to a supercharge of the form 
\[
Q = q_+ \otimes w_1 \in S_+ \otimes W_+ .
\]
Such a $Q$ is preserved under the twisting homomorphism
\[
\MU(3) \to {\rm Spin}(6) \times \SU(2) .
\]
In the first component this is the homomorphism $\MU(3) \to {\rm Spin}(6)$ lifting $\U(3) \to {\rm SO}(6)$. 
In the second component, this is the composition $\MU(3) \to \U(1) \to \SU(2)$ where the first map is $\det^{-1/2}$ and the second map is $z \mapsto \begin{pmatrix} z & 0 \\ 0 & \Bar{z} \end{pmatrix}$. 

There is no twisting datum homomorphism $\alpha : U(1) \to \SU(2)$ compatible with this twisting homomorphism.
Thus, the resulting twisted theory is only $\ZZ/2$-graded.

Under the action of $\MU(3)$, the fields decompose as:
\begin{align*}
c & \mapsto A_0 \in \Omega^{0,0}(\CC^3 ; \fg) \\
A & \mapsto A_{0,1} + A_{1,0} \in \Omega^{0,1}(\CC^3 ; \fg) \oplus \Omega^{1,0}(\CC^3 ; \fg) \\
\lambda & \mapsto \lambda_{0} + {B}_{1,0} + \chi + \lambda_0 \in \Omega^0(\CC^3 ; \fg) \oplus \Omega^{0,2}(\CC^3 ; \fg) \oplus \Omega^{3,0}(\CC^3 ; \fg) \oplus \Omega^{3,2}(\CC^3 ; \fg) \\
\phi & \mapsto \nu + \psi_0 \in \Omega^0(\CC^3; K^{-1/2} \otimes U) \oplus \Omega^0(\CC^3 ; K^{1/2} \otimes U) \\
\psi & \mapsto \psi_{0,1} + \nu' \in \Omega^{0,1}(\CC^3; K^{1/2} \otimes U) \oplus \Omega^{0,3} (\CC^3; K^{1/2} \otimes U ),
\end{align*} 
...
\begin{thm}
The holomorphic twist of 6d $\mc N=(1, 0)$ super Yang--Mills with Lie algebra $\fg$ and matter valued in the symplectic representation $U$ is $\MU(3)$-equivariantly equivalent to holomorphic Chern-Simons on $\CC^3$ with values in $U \ham g$, with classical moduli space $\mr{Map}(\CC^3, U\ham \gg)$.
...
\begin{theorem}
\chris{Rank $(1,1)$ deforming to rank $(2,1)$, the former is done above, the deformation maybe doesn't come from dimensional reduction?} $\mr{Map}(\CC_{\mr{Dol}} \times \RR^2_{\mr{dR}}, B\gg_{\mr{Hod}})$.
\end{theorem}
...
\chris{todo: $(1,0)$, $(2,0)$, $(1,1)$ as above, with $U = T^*\gg$.  Then $(2,1)$ and generic $(2,2)$ arise as deformations of $(1,1)$ by dimensional reduction from 5d.  Finally the special $(2,2)$ arises by dimensional reduction from the 5d B-twist.}
