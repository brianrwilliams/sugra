...
We call $\cE$ the {\bf space of BV fields}.
...
\subsubsection{Super Yang--Mills Theory}
...
\end{enumerate}
This construction only works in the four special dimensions $3,4,6$ and 10: our argument will use results of Baez and Huerta \cite{BaezHuerta} proven using the structure of the four normed division algebras $\RR, \CC, \bb H$ and $\bb O$.  In particular, we will use the following ``3-$\psi$'s rule'' \chris{see citations in Baez Huerta for more references, which we might include}.  Let $g(v,w)$ denote the Riemannian metric on $\RR^n$.
%We will write $\wt \Sigma$ for the spin representation obtained from $\Sigma$ by parity reversal, so in dimensions 3 and 4 $\wt \Sigma = \Sigma$, in dimension 6 $\wt \Sigma = S_- \otimes W$, and in dimension 10 $\wt \Sigma = S_-$.

\begin{theorem}[{\cite[Theorem 11]{BaezHuerta}}]
The multilinear map 
...
\end{align*}
Here, the notation $\sd F_A$ stands for the iterated Clifford multiplication $\sd F_A = F_{ij} \gamma^i \gamma^j$.  We can check that this supersymmetry action does not close off-shell.  Indeed \chris{...}

\chris{heavily edit the below.}
...
\chris{we'll have to see whether we want Poincar\'e or just translation here, probably translation, because then there are no $\d_{\mr{Lie}}I^{(i)}$ terms to worry about.  On the other hand we would like to know that the theories are supersymmetric, not just super-translation invariant.  To include rotations and R-symmetries we need to additionally check that some terms like $\d_{\mr{Lie}}+ \{I_{\mr{rotation}} + I_{\text{R-symmetry}}, I^{(i)}\}$ vanish.}
...
\chris{apart from the Lemma \ref{10d_onshell_lemma}, this is all completely identical for dimensions 3,4,6,10.  Even that lemma only uses abstract gamma matrix manipulations that work in any dimension along with the Baez Huerta rule that applies in those dimensions, so I think we should just state a single result valid for those four theories simultaneously.}
\end{proof}
