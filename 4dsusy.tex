...
\brian{I'd also  like the following  to be true.
\begin{prop}
Suppose $Q_+ \in S_+$, $Q_-\in S_-$, and $\psi_+ \in C^\infty(V , S_+)$. 
Then
\[
\sd \dd (\rho(\Gamma(Q_+, Q_-)) \psi_+) = \rho(\d(Q_+, \psi_+)) Q_- + (Q_-, \sd \dd \psi_+)Q_+ .
\]
\end{prop}
}
...
\]
This is zero by Proposition \brian{3-psi}. 
