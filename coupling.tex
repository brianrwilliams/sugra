...
\begin{align*}
\{I_{\rm couple}, I^{(2)}_{\rm matter}\} = & (\rho(\Gamma(Q_1,Q_2)) \psi^*, [A, \psi]) + (\lambda, \mu(\phi, \rho(\Gamma(Q_1,Q_2)) \psi^*)) +  (\psi^*, [c, \rho(\Gamma(Q_1,Q_2)) \psi^*])
\end{align*}

Expanding $\{I^{(1)}_{\rm gauge}, I^{(1)}_{\rm couple}\}$:
\begin{align*}
\{I^{(1)}_{\rm gauge}, I^{(1)}_{\rm couple}\} = (\phi^* Q_1 , \rho([A,\phi]) Q_2) + \brian{finish}
\end{align*}
