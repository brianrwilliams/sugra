...
\begin{enumerate}
 ...
 Observe that the classical BV complex associated to this action functional can be written in the following form:
 \[\xymatrix{
 &&\Omega^{1,0}(\CC^5;\gg)_\psi \ar[dl] \ar[dr] \ar[r] &\Omega^{1,0}(\CC^5;\gg)_{A_{1,0}} \ar[dr]\\
 \Omega^0(\CC^5;\gg)_c \ar[r] \ar[urrr] &\Omega^{0,1}(\CC^5;\gg)_{A_{0,1}}  \ar[dl] \ar[r] &\Omega^{0,2}(\CC^5;\gg)_{B} \ar[dl]\ar[r] &\Omega^{0,3}(\CC^5;\gg)_{B^\vee} \ar[r] &\Omega^{0,4}(\CC^5;\gg)_{A_{0,1}^\vee} \ar[dlll] \ar[r] &\Omega^{0,5}(\CC^5;\gg)_{c^\vee} \\
 \Omega^{4,0}(\CC^5;\gg)_{A_{1,0}^\vee} \ar[r] \ar[urrrrr]  &\Omega^{4,0}(\CC^5;\gg)_{\psi^\vee}. 
 }\]
 \chris{I'm confused here: there are actually both maps into the $\psi \to A_{1,0}$ two step complex ($c \mapsto \dd c$) and maps out ($A_{1,0} \to A_{0,1}^\vee$ and $\psi \to B^\vee$).  So the argument we'd been thinking of doesn't work?  I don't remember, did we discuss this? I want to eliminate those ugly arrows involving $c$ and $c^\vee$ (also obviously they make the diagram above unreadable).}
