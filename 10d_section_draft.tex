...
\begin{enumerate}
 \item The first part of our equivalence of classical field theories will be a simple change of variables.  Let $\chi'^\vee = \chi^\vee + \Lambda F_{1,1}$.  In terms of $\chi'^\vee$, the quadratic part of the twisted action functional becomes
 \begin{align*}
  S^Q &= \int \d^5z \left(\Lambda^2 \langle \ol \dd A_{0,1} \wedge \dd A_{1,0} \rangle + \frac 12 |\chi'^\vee|^2 + \Lambda( \chi \wedge (\ol \dd \psi))  + \Lambda^2(B \wedge (\dd \psi))\right) \Omega + (B \wedge \ol \dd B) \\
 ...
 Observe that the classical BV complex associated to this action functional takes the form \chris{I'm confused here: there are actually both maps into the $\psi \to A_{1,0}$ two step complex ($c \mapsto \dd c$) and maps out ($A_{1,0} \to A_{0,1}^\vee$ and $\psi \to B^\vee$).  So the argument we'd been thinking of doesn't work?  I don't remember, did we discuss this?}
 
 \item Now, let's understand how the interaction functional behaves under this equivalence of classical field theories. \chris{...}
