...
\delta_Q \lambda &= \frac 14 \sd F_A Q,
\end{align*}
where the notation $\sd F_A$ stands for the iterated Clifford multiplication $\sd F_A = F^{ij} \gamma_i \gamma_j$.  To check that this defines a supersymmetry action we need to check it's compatible with the brackets in the supersymmetry algebra.  So, we calculate
\begin{align*}
[\delta_{Q_1}, \delta_{Q_2}] A &= \frac 14 (\Gamma(Q_2,\sd F_A Q_1) - \Gamma(Q_1,\sd F_A Q_2)) \\
&= \\
\text{and } \delta_{Q_1}, \delta_{Q_2}] \lambda &= \frac 14 (\sd F_{\Gamma(Q_2, \lambda)} Q_1 - \sd F_{\Gamma(Q_1,\lambda)} Q_2) \\
&= 
\end{align*}
